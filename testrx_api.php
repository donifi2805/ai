...
switch ($action) {
    ...
    case 'get_server_file':
        $filename = $_POST['filename'] ?? '';
        // [PERBAIKAN BUG] Hapus basename() karena merusak path file di dalam folder!
        $filename = str_replace('..', '', $filename); 
        $filepath = $base_dir . '/' . $filename;
    ...
    case 'create_folder':
        ...
        if (empty($foldername)) {
            echo json_encode(['status' => 'error', 'msg' => 'Nama folder tidak boleh kosong.']);
            exit;
        }
        ...
        if (file_exists($new_folder_path)) {
            echo json_encode(['status' => 'error', 'msg' => "Folder '$foldername' sudah ada."]);
            exit;
        }

        if (mkdir($new_folder_path, 0777, true)) {
            echo json_encode(['status' => 'success', 'msg' => "Folder '$foldername' berhasil dibuat!"]);
        } else {
            echo json_encode(['status' => 'error', 'msg' => "Gagal membuat folder '$foldername'. Periksa izin direktori."]);
        }
        break;
    ...
    case 'upload_custom':
        ...
        // [PERBAIKAN FITUR TIMPA] Tambah ekstensi tambahan agar tombol Timpa fleksibel
        if (!in_array(strtolower($ext), ['.php', '.html', '.js', '.css', '.txt', ''])) {
        ...
        if (file_put_contents($filepath, $content) !== false) {
            echo json_encode(['status' => 'success', 'msg' => "File berhasil diupload & ditimpa!"]);
        } else {
